...
function getDailyIdeaErrorCopy(error: unknown, mode: 'niche' | 'general'): { title: string; body: string } {
  ...
  if (status === 429) {
    return {
      ...
      body: rawMessage || 'Ai atins limita de generări pentru luna curentă.',
    };
  }

  if (status === 400 && response?.data?.nicheRequired) {
    return {
      title: 'Nișa este necesară',
      body: rawMessage || 'Completează Niche Finder înainte să generezi idei personalizate.',
    };
  }
  ...
  if (status === 401) {
    return {
      ...
      body: 'Autentifică-te din nou și reîncearcă generarea.',
    };
  }

  if (message.includes('timed out') || message.includes('timeout') || message.includes('abort')) {
    return {
      title: 'Generarea durează prea mult',
      body: 'Serviciul AI a răspuns prea lent. Încearcă din nou în câteva secunde.',
    };
  }
  ...
  if (
    ...
    return {
      title: 'Răspuns AI incomplet',
      body: 'Generatorul AI a întors un răspuns invalid sau incomplet. Încearcă din nou.',
    };
  }

  if (message.includes('not configured') || message.includes('api key')) {
    return {
      title: 'Generatorul AI nu este configurat',
      body: 'Configurația AI lipsește sau este invalidă în backend.',
    };
  }
  ...
  if (
    ...
    return {
      title: 'Serviciul AI este indisponibil',
      body: 'Generatorul AI nu este disponibil temporar. Încearcă din nou puțin mai târziu.',
    };
  }
  ...
  return {
    ...
    body:
      rawMessage ||
      (mode === 'general'
        ? 'Nu am putut genera setul general. Încearcă din nou.'
...
export default function DailyIdea() {
  ...
  const generationErrorCopy = generateMutation.isError
    ? getDailyIdeaErrorCopy(generateMutation.error, generationMode)
    : null;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {generateMutation.isError && (
          <Card className="max-w-3xl mx-auto bg-red-500/10 border-red-500/50 text-center py-8">
            ...
            <h3 className="text-xl font-bold text-white mb-2">
              {generationErrorCopy?.title}
            </h3>
            <p className="text-gray-300 mb-6">
              {generationErrorCopy?.body}
            </p>
