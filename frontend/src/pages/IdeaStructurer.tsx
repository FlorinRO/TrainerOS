...
function getStructuredIdeaErrorCopy(error: unknown): string {
  ...
  if (status === 429) {
    return rawMessage || 'Ai atins limita lunară pentru idei structurate.';
  }

  if (status === 400 && response?.data?.nicheRequired) {
    return rawMessage || 'Completează Niche Finder înainte să structurezi ideea.';
  }
  ...
  if (message.includes('timed out') || message.includes('timeout') || message.includes('abort')) {
    return 'Structurarea durează prea mult. Serviciul AI a răspuns prea lent; încearcă din nou.';
  }
  ...
  if (
    ...
    return 'Serviciul AI a întors un răspuns incomplet sau invalid. Încearcă din nou.';
  }

  if (message.includes('not configured') || message.includes('api key')) {
    return 'Generatorul AI nu este configurat corect în backend.';
  }
  ...
  if (
    ...
    return 'Serviciul AI este indisponibil temporar. Încearcă din nou puțin mai târziu.';
  }

  return rawMessage || 'Nu am putut structura ideea.';
}
...
export default function IdeaStructurer() {
  ...
  const structuredIdeaError = structureMutation.isError
    ? getStructuredIdeaErrorCopy(structureMutation.error)
    : null;
