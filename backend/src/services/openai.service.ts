...
const DAILY_IDEA_ALLOWED_FORMATS = ['REEL', 'CAROUSEL', 'STORY'] as const;
...
function normalizeSceneArray(value: unknown): Scene[] {
  ...
  return value
    .map((item, index) => {
      ...
      const text = normalizeTextValue(source.text);
      const visual = normalizeTextValue(source.visual) || 'Cadru simplu, clar și ușor de filmat';
      const sceneNumber = Number(source.scene);

      return {
        scene: Number.isFinite(sceneNumber) && sceneNumber > 0 ? sceneNumber : index + 1,
        text,
        visual,
      };
    })
...
function looksLikeIdeaPlaceholder(value: string): boolean {
...
function normalizeDailyIdeaResult(value: unknown, fallbackFormat: DailyIdeaResult['format']): DailyIdeaResult {
  const source = value && typeof value === 'object' ? (value as Record<string, unknown>) : {};
  const rawFormat = normalizeTextValue(source.format).toUpperCase() as DailyIdeaResult['format'];
  const format = DAILY_IDEA_ALLOWED_FORMATS.includes(rawFormat) ? rawFormat : fallbackFormat;
  const script = normalizeSceneArray(source.script);
  const conversionRate = Number(source.conversionRate);

  return {
    format,
    hook: normalizeTextValue(source.hook),
    script,
    ...
    leadMagnet: normalizeTextValue(source.leadMagnet),
    dmKeyword: normalizeTextValue(source.dmKeyword),
    reasoning: normalizeTextValue(source.reasoning),
  };
}

function isDailyIdeaResultIncomplete(result: DailyIdeaResult): boolean {
  if (!DAILY_IDEA_ALLOWED_FORMATS.includes(result.format)) {
    return true;
  }

  if (!result.hook || looksLikeIdeaPlaceholder(result.hook) || result.hook.split(/\s+/).length < 3) {
    return true;
  }

  if (result.script.length < 3) {
    return true;
  }

  if (
    result.script.some((scene) => {
      const text = scene.text.trim();
      return !text || looksLikeIdeaPlaceholder(text) || text.split(/\s+/).length < 20;
    })
  ) {
    return true;
  }

  if (!result.cta || looksLikeIdeaPlaceholder(result.cta)) {
    return true;
  }

  if (!result.leadMagnet || looksLikeIdeaPlaceholder(result.leadMagnet)) {
    return true;
  }

  if (!result.dmKeyword || looksLikeIdeaPlaceholder(result.dmKeyword)) {
    return true;
  }

  return !result.reasoning || looksLikeIdeaPlaceholder(result.reasoning);
}

function buildEmergencyDailyIdeaResult(
  format: DailyIdeaResult['format'],
  niche: string,
  topic: string,
  dmKeyword: string
): DailyIdeaResult {
  const result = buildMockDailyIdeaResult(format, niche, topic, dmKeyword);
...
function buildEmergencyMultiFormatIdeaResult(input: DailyIdeaInput): MultiFormatIdeaResult {
  const niche = normalizeIdeaSeed(input.niche, 'fitness general pentru adulți ocupați');

  return {
    reel: buildEmergencyDailyIdeaResult(
      'REEL',
      niche,
      'de ce oamenii renunță când planul pare prea greu',
      'PLAN'
    ),
    carousel: buildEmergencyDailyIdeaResult(
      'CAROUSEL',
      niche,
      'cum simplifici procesul ca să apară consistența',
      'CLAR'
    ),
    story: buildEmergencyDailyIdeaResult(
      'STORY',
      niche,
      'ce greșeală mică îți taie progresul de la început',
      'START'
    ),
  };
}

async function parseDailyIdeaResponse(
  content: string,
  fallbackFormat: DailyIdeaResult['format']
): Promise<DailyIdeaResult> {
  const parsed = await parseModelJson<DailyIdeaResult>(content);
  return normalizeDailyIdeaResult(parsed, fallbackFormat);
}
...
function isMultiFormatIdeaResultIncomplete(result: MultiFormatIdeaResult): boolean {
  return (
    isDailyIdeaResultIncomplete(result.reel) ||
    isDailyIdeaResultIncomplete(result.carousel) ||
    isDailyIdeaResultIncomplete(result.story)
  );
}
...
function buildMockDailyIdeaResult(
  ...
  const nicheText = normalizeIdeaSeed(niche, 'fitness pentru oameni ocupați');
  const topicText = normalizeIdeaSeed(topic, 'cum faci progres fără să complici totul');

  return {
    format,
    hook:
      format === 'REEL'
        ? `Dacă ești în ${nicheText}, oprește greșeala asta`
        : format === 'CAROUSEL'
          ? `${nicheText}: ce să faci când simți că te blochezi`
          : `În ${nicheText}, asta îți sabotează progresul`,
    script:
      format === 'CAROUSEL'
        ? [
            {
              scene: 1,
              text: `Mulți oameni din ${nicheText} cred că problema este motivația. De fapt, de multe ori problema este că încearcă să facă prea multe schimbări în același timp. Când totul pare greu de ținut, renunțarea vine repede. Ideea acestui carusel este să arate o variantă mai simplă și mai realistă.`,
              visual: 'Copertă cu hook clar și fundal simplu de sală sau birou',
            },
            {
              scene: 2,
              text: `Primul pas este să alegi un singur comportament care chiar contează. De exemplu, nu schimbi tot planul alimentar într-o zi, ci începi cu o masă mai clară sau cu un program fix pentru antrenament. Asta scade presiunea și crește șansa să rămâi consecvent.`,
              visual: 'Slide cu un singur pas evidențiat clar',
            },
            {
              scene: 3,
              text: `Al doilea pas este să legi acțiunea de ceva ce faci deja. Dacă dimineața bei cafea la aceeași oră, poți lega de acel moment o scurtă plimbare sau pregătirea meselor. Când un obicei nou are deja un loc clar în zi, apare mai puțină rezistență.`,
              visual: 'Rutina zilnică simplificată în 2-3 repere',
            },
            {
              scene: 4,
              text: `Al treilea pas este să urmărești progresul simplu, nu perfect. Nu ai nevoie de tabele complicate. Uneori este suficient să vezi că ai repetat comportamentul de 3-4 ori într-o săptămână. Asta îți dă dovada că ești pe direcția bună.`,
              visual: 'Checklist scurt sau calendar minimalist',
            },
            {
              scene: 5,
              text: `Aici apare diferența dintre oamenii care pornesc tare și cei care chiar ajung la rezultat. Cei care reușesc își fac procesul mai ușor, nu mai spectaculos. Dacă un pas e prea greu, îl micșorezi, nu abandonezi complet.`,
              visual: 'Comparație între strategie haotică și strategie simplă',
            },
            {
              scene: 6,
              text: `Dacă vrei, poți transforma asta într-un mesaj foarte bun de content: nu promiți miracole, ci claritate. Exact genul de mesaj care atrage oameni serioși, nu doar curioși. Asta îl face util atât pentru audiență, cât și pentru conversie.`,
              visual: 'Exemplu de mesaj de content notat pe ecran',
            },
            {
              scene: 7,
              text: `Concluzia este simplă: progresul vine mai repede când procesul este clar și repetabil. Dacă oamenii înțeleg ce au de făcut chiar de azi, cresc șansele să aibă rezultate și să aibă încredere în tine ca antrenor.`,
              visual: 'Slide de concluzie cu mesaj scurt și clar',
            },
            {
              scene: 8,
              text: `Dacă vrei o variantă pe care s-o aplici direct în content sau cu clienții tăi, trimite ${dmKeyword} și îți dau un exemplu gata structurat. E făcut ca să fie simplu de folosit, fără teorie inutilă și fără pași complicați.`,
              visual: 'CTA final cu keyword mare pe ecran',
            },
          ]
        : format === 'STORY'
          ? [
              {
                scene: 1,
                text: `Dacă ești în ${nicheText}, nu lipsa de voință te blochează prima. De multe ori problema este că totul pare prea complicat încă din start.`,
                visual: 'Selfie vertical cu text mare pe ecran',
              },
              {
                scene: 2,
                text: `Când planul are prea multe reguli, oamenii îl țin 2-3 zile și apoi renunță. Nu pentru că nu vor, ci pentru că nu încape natural în viața lor reală.`,
                visual: 'Cadru rapid cu program aglomerat sau checklist încărcat',
              },
              {
                scene: 3,
                text: `Mai bine alegi un pas mic, clar și repetabil. Acolo apare consistența, iar consistența aduce rezultat real.`,
                visual: 'Cadru scurt cu un obicei simplu executat',
              },
              {
                scene: 4,
                text: `Dacă vrei un exemplu concret pe tema ${topicText.toLowerCase()}, trimite ${dmKeyword} și îți trimit structura pe scurt.`,
                visual: 'CTA clar, full screen',
              },
            ]
          : [
              {
                scene: 1,
                text: `Dacă lucrezi cu oameni din ${nicheText}, una dintre cele mai mari greșeli este să le dai prea mult deodată. Când omul pleacă cu zece reguli noi, simte că trebuie să devină altă persoană peste noapte. Acolo apare blocajul, nu progresul. Mai întâi ai nevoie de claritate, nu de complexitate.`,
                visual: 'Cadru frontal cu antrenorul vorbind direct către cameră',
              },
              {
                scene: 2,
                text: `Uite un exemplu simplu. În loc să schimbi tot programul, alegi un singur punct critic: micul dejun, pașii zilnici sau două antrenamente fixe pe săptămână. Când un lucru este clar, omul îl poate repeta. Iar când îl poate repeta, începe să creadă că poate duce procesul până la capăt.`,
                visual: 'Cadru cu gesturi naturale și text-cheie pe ecran',
              },
              {
                scene: 3,
                text: `Exact aici devine util și contentul tău. Dacă mesajul transmite că progresul vine din pași simpli și consecvenți, atragi oamenii potriviți. Nu atragi doar persoane care vor o motivație de moment, ci oameni care chiar vor un proces pe care să-l urmeze.`,
                visual: 'Cadru cu exemple notate pe telefon sau tabletă',
              },
              {
                scene: 4,
                text: `De aceea tema ${topicText.toLowerCase()} merge bine în content. Este clară, practică și ușor de aplicat chiar azi. Nu sună spectaculos forțat, dar sună real. Și exact asta construiește încredere mai repede decât orice promisiune mare.`,
                visual: 'Cadru de sus cu bullet points simple pe ecran',
              },
            ],
    cta: `Trimite ${dmKeyword} și îți trimit un exemplu simplu, clar și aplicat pe ${nicheText.toLowerCase()}.`,
...
function buildMockMultiFormatIdeaResult(input: DailyIdeaInput): MultiFormatIdeaResult {
  const niche = normalizeIdeaSeed(input.niche, 'fitness general pentru adulți ocupați');
  return {
    reel: buildMockDailyIdeaResult('REEL', niche, 'de ce oamenii renunță când planul pare prea greu', 'PLAN'),
    carousel: buildMockDailyIdeaResult('CAROUSEL', niche, 'cum simplifici procesul ca să apară consistența', 'CLAR'),
    story: buildMockDailyIdeaResult('STORY', niche, 'ce greșeală mică îți taie progresul de la început', 'START'),
  };
}

function buildMockStructuredIdeaResult(input: {
  ideaText: string;
  niche: string;
}): StructuredIdeaResult {
  const ideaSeed = normalizeIdeaSeed(input.ideaText, 'ideea ta inițială');
  const niche = normalizeIdeaSeed(input.niche, 'nișa ta');

  return {
    mainIdea: `Ideea principală este că ${ideaSeed.toLowerCase()} trebuie explicat simplu și aplicat clar pentru oameni din ${niche.toLowerCase()}.`,
    hooks: [
      `De ce ${ideaSeed.toLowerCase()} blochează mai mulți oameni decât crezi`,
      `${niche}: cum explici clar ${ideaSeed.toLowerCase()}`,
    ],
    script: [
      {
        sectionTitle: 'PARTEA 1 – Context',
        text: `Mulți oameni aud ideea asta și cred că au înțeles-o din prima, dar în realitate o interpretează vag. Când mesajul rămâne prea general, omul nu știe ce să facă diferit chiar de azi. De aceea, primul pas este să pui problema într-un context real, recognoscibil pentru cineva din ${niche.toLowerCase()}. Aici contează să vorbești simplu, fără teorie inutilă și fără formulări pompoase.`,
      },
      {
        sectionTitle: 'PARTEA 2 – Explicație clară',
        text: `Partea centrală trebuie să explice clar de ce ${ideaSeed.toLowerCase()} contează practic. Nu spui doar că este important, ci arăți unde apare blocajul, cum îl vezi în viața reală și de ce oamenii rămân blocați acolo. Când explici în limbaj simplu, mesajul devine imediat mai util și mai ușor de reținut. Asta este diferența dintre un mesaj interesant și unul care chiar produce acțiune.`,
      },
      {
        sectionTitle: 'PARTEA 3 – Exemplu / aplicație',
        text: `După explicație, ai nevoie de un exemplu concret. Poți arăta o situație din programul zilnic al clientului, o greșeală frecventă sau o decizie aparent mică ce schimbă rezultatul. Exemplul face ideea ușor de vizualizat și îi arată omului că nu vorbești teoretic. În același timp, îți crește credibilitatea pentru că mesajul sună ca ceva văzut și lucrat cu oameni reali.`,
      },
      {
        sectionTitle: 'PARTEA 4 – Principiu final',
        text: `Finalul trebuie să lege totul într-un principiu simplu. Omul trebuie să rămână cu o concluzie clară și cu senzația că știe ce să facă mai departe. Dacă mesajul este ușor de repetat și de pus în practică, șansele să fie salvat, distribuit sau să genereze răspuns cresc mult. Exact aici apare diferența dintre content care doar sună bine și content care mută omul spre acțiune.`,
      },
    ],
    cta: `Dacă vrei, îți pot transforma ideea într-un script clar, natural și ușor de filmat pentru ${niche.toLowerCase()}.`,
    ctaStyleApplied: 'direct și conversațional',
    improvements: [
      'Mesaj clarificat',
...
export async function generateDailyIdea(input: DailyIdeaInput): Promise<DailyIdeaResult> {
  if (MOCK_AI_RESPONSES) {
    return buildMockDailyIdeaResult(
      'REEL',
      input.niche,
      'de ce oamenii se blochează când încearcă să facă prea mult deodată',
      'PLAN'
    );
  }
...
FORMAT:
{
  ...
  const generateAndParse = async (): Promise<DailyIdeaResult> => {
    const content = await generateAnthropicJson(prompt, 0.8, 3500);
    console.log(`✅ Daily idea response received (${content.length} chars)`);
    return parseDailyIdeaResponse(content, 'REEL');
  };

  try {
    let result: DailyIdeaResult;

    try {
      result = await generateAndParse();
    } catch (error) {
      console.warn('Daily idea parsing failed on first attempt, retrying once:', summarizeAiError(error));
      result = await generateAndParse();
    }

    if (isDailyIdeaResultIncomplete(result)) {
      console.warn('Daily idea response incomplete after parsing, using emergency fallback.');
      return buildEmergencyDailyIdeaResult(
        'REEL',
        input.niche,
        'de ce oamenii se blochează când încearcă să facă prea mult deodată',
        'PLAN'
    ...
    console.error('Daily idea generation failed, returning emergency fallback:', summarizeAiError(error));
    return buildEmergencyDailyIdeaResult(
      'REEL',
      input.niche,
      'de ce oamenii se blochează când încearcă să facă prea mult deodată',
...
export async function generateMultiFormatIdea(input: DailyIdeaInput): Promise<MultiFormatIdeaResult> {
  if (MOCK_AI_RESPONSES) {
    return buildMockMultiFormatIdeaResult(input);
  }
...
FORMAT:
{
  ...
  const parseResponse = async (): Promise<MultiFormatIdeaResult> => {
    const content = await generateAnthropicJson(prompt, 0.8, 6000);
    console.log(`✅ Multi-format response received (${content.length} chars)`);
    return normalizeMultiFormatIdeaResult(await parseModelJson<MultiFormatIdeaResult>(content));
  };
  ...
  try {
    ...
    console.warn('Multi-format idea parsing failed on first attempt, retrying generation once:', summarizeAiError(error));
    try {
      result = await parseResponse();
    } catch (retryError) {
      console.error(
        'Multi-format idea generation failed after retry, using emergency fallback:',
        summarizeAiError(retryError)
      );
      return buildEmergencyMultiFormatIdeaResult(input);
    }
  }

  if (isMultiFormatIdeaResultIncomplete(result)) {
    console.warn('Multi-format idea response incomplete after parsing, using emergency fallback.');
    return buildEmergencyMultiFormatIdeaResult(input);
  }
...
export async function structureUserIdea(input: {
  ...
  if (MOCK_AI_RESPONSES) {
    return buildMockStructuredIdeaResult(input);
  }
...
function normalizeNicheFragment(value: string, fallback: string): string {
  const normalized = value.replace(/\s+/g, ' ').trim();
  return normalized || fallback;
}

function translateSituationToNicheLanguage(value: string): string {
  const normalized = value.trim().toLowerCase();

  if (normalized.includes('ocupati') || normalized.includes('ocupați')) {
    return 'au un program haotic, puțin timp și le este greu să se organizeze';
  }

  if (normalized.includes('estetic') && normalized.includes('nu se tin')) {
    return 'vor să arate mai bine, dar nu reușesc să țină un proces suficient de mult';
  }

  if (normalized.includes('la inceput') || normalized.includes('la început')) {
    return 'sunt la început și au nevoie de pași clari, fără să se simtă copleșite';
  }

  if (normalized.includes('nu au structura') || normalized.includes('nu au structură')) {
    return 'știu în mare ce ar trebui să facă, dar nu au structură și claritate';
  }

  if (normalized.includes('dureri') || normalized.includes('limitari') || normalized.includes('limitări')) {
    return 'au dureri sau limitări fizice și le este teamă să înceapă greșit';
  }

  return normalizeNicheFragment(value, 'au nevoie de un proces clar și realist');
}

function translateProblemToNicheLanguage(value: string): string {
  const normalized = value.trim().toLowerCase();

  if (normalized.includes('consecven')) {
    return 'abandonează după una-două săptămâni și nu reușesc să rămână consecvente';
  }

  if (normalized.includes('energie')) {
    return 'se simt obosite constant și nu au energie pentru antrenament sau alimentație organizată';
  }

  if (normalized.includes('confuz')) {
    return 'sunt confuze și nu știu exact ce să facă, de unde să înceapă sau ce să creadă';
  }

  if (normalized.includes('alimentatia') || normalized.includes('alimentația')) {
    return 'mănâncă haotic și nu reușesc să-și organizeze mesele suficient de bine';
  }

  if (normalized.includes('frica') || normalized.includes('rusinea') || normalized.includes('rușinea')) {
    return 'au frică sau rușine de sală și amână începutul';
  }

  return normalizeNicheFragment(value, 'au un blocaj recurent care le oprește progresul');
}

function translateOutcomeToNicheLanguage(value: string): string {
  const normalized = value.trim().toLowerCase();

  if (normalized.includes('sa se tina constant') || normalized.includes('să se țină constant')) {
    return 'consistență în antrenamente și alimentație';
  }

  if (normalized.includes('sa slabeasca') || normalized.includes('să slăbească')) {
    return 'slăbire sustenabilă';
  }

  if (normalized.includes('tonifieze') || normalized.includes('arate mai bine')) {
    return 'tonifiere și schimbare vizibilă a aspectului fizic';
  }

  if (normalized.includes('mai multa energie') || normalized.includes('mai multă energie')) {
    return 'mai multă energie și un stil de viață mai ușor de dus';
  }

  if (normalized.includes('scape de dureri')) {
    return 'mai puține dureri și mai multă încredere în mișcare';
  }

  return normalizeNicheFragment(value, 'rezultate sustenabile');
}

function looksTooGenericNicheVariantTitle(value: string): boolean {
  const normalized = value.trim().toLowerCase();
  if (!normalized) {
    return true;
  }
  ...
  if (genericStarts.some((prefix) => normalized === prefix || normalized.startsWith(`${prefix} `))) {
    return true;
  }

  const hasAudience =
    normalized.includes('femei') ||
    normalized.includes('barbati') ||
    normalized.includes('bărbați') ||
    normalized.includes('mame') ||
    normalized.includes('adulti') ||
    normalized.includes('adulți') ||
    /\d{2}\+/.test(normalized) ||
...
function buildAudienceLabel(input: NicheDiscoverPhaseAInput): string {
  const ageLabel = input.ageRanges.length ? input.ageRanges.join(', ') : '25-45';

  if (input.gender === 'femei') {
    return `femei ${ageLabel}`;
  }

  if (input.gender === 'barbati') {
    return `bărbați ${ageLabel}`;
  }

  return `adulți ${ageLabel}`;
}

function buildAudienceNicheLabel(audience: string): string {
  const normalized = audience.toLowerCase();

  if (normalized.startsWith('femei ')) {
    return audience;
  }

  if (normalized.startsWith('bărbați ') || normalized.startsWith('barbati ')) {
    return audience;
  }
...
function buildOutcomeNicheLabel(outcome: string): string {
  const normalized = outcome.toLowerCase();

  if (normalized.includes('consistență')) {
    return 'Consistență în sală și alimentație';
  }

  if (normalized.includes('slăbire') || normalized.includes('slabire')) {
    return 'Slăbire sustenabilă';
  }

  if (normalized.includes('tonifiere')) {
    return 'Tonifiere și schimbare vizibilă';
  }

  if (normalized.includes('energie')) {
    return 'Mai multă energie și structură';
  }

  if (normalized.includes('dureri')) {
    return 'Mișcare fără dureri și mai multă încredere';
  }

  return outcome.charAt(0).toUpperCase() + outcome.slice(1);
}

function buildContextNicheLabel(situation: string): string {
  const normalized = situation.toLowerCase();

  if (normalized.includes('program haotic') || normalized.includes('puțin timp')) {
    return 'cu program haotic și puțin timp pentru ele';
  }

  if (normalized.includes('la început')) {
    return 'care încep de la zero și se simt copleșite';
  }

  if (normalized.includes('structură') || normalized.includes('claritate')) {
    return 'care știu în mare ce să facă, dar nu au structură';
  }

  if (normalized.includes('dureri') || normalized.includes('limitări')) {
    return 'care au dureri sau limitări și ezită să înceapă';
  }

  return 'care au nevoie de o direcție clară și realistă';
}

function buildProblemNicheLabel(problem: string): string {
  const normalized = problem.toLowerCase();

  if (normalized.includes('abandonează') || normalized.includes('consecvente')) {
    return 'abandonează după una-două săptămâni';
  }

  if (normalized.includes('obosite') || normalized.includes('energie')) {
    return 'se simt fără energie aproape zilnic';
  }

  if (normalized.includes('confuze') || normalized.includes('nu știu exact')) {
    return 'sunt confuze și nu știu de unde să înceapă';
  }

  if (normalized.includes('mănâncă haotic')) {
    return 'mănâncă haotic și nu reușesc să se organizeze';
  }

  if (normalized.includes('frică') || normalized.includes('rușine')) {
    return 'amână începutul din frică sau rușine';
  }

  return 'au un blocaj care le oprește progresul';
}

function buildSpecificFallbackNicheTitle(
  audience: string,
  situation: string,
  problem: string,
  outcome: string,
  ...
  const cleanAudience = buildAudienceNicheLabel(normalizeNicheFragment(audience, 'adulți 25-45'));
  const cleanSituation = buildContextNicheLabel(
    normalizeNicheFragment(situation, 'au nevoie de o direcție clară și realistă')
  );
  const cleanProblem = buildProblemNicheLabel(
    normalizeNicheFragment(problem, 'au un blocaj care le oprește progresul')
  );
  const cleanOutcome = buildOutcomeNicheLabel(
    normalizeNicheFragment(outcome, 'rezultate sustenabile')
  );

  if (style === 0) {
    return `${cleanOutcome} pentru ${cleanAudience} ${cleanSituation}`;
  }

  if (style === 1) {
    return `${cleanOutcome} pentru ${cleanAudience} care ${cleanProblem.toLowerCase()}`;
  }

  return `${cleanOutcome} pentru ${cleanAudience} care vor progres fără să mai ${cleanProblem.toLowerCase()}`;
}
...
function buildFallbackNicheVariants(input: NicheDiscoverPhaseAInput): NicheVariant[] {
  ...
  const topSituation = translateSituationToNicheLanguage(
    input.valueSituations[0] || '',
  );
  const secondSituation = translateSituationToNicheLanguage(
    input.valueSituations[1] || topSituation,
  );
  const topProblem = translateProblemToNicheLanguage(
    input.commonProblems[0] || '',
  );
  const secondProblem = translateProblemToNicheLanguage(
    input.commonProblems[1] || topProblem,
  );
  const topOutcome = translateOutcomeToNicheLanguage(input.primaryOutcome);

  return [
    {
      variant: buildSpecificFallbackNicheTitle(audience, topSituation, topProblem, topOutcome, 0),
      description: `Pentru persoane din segmentul ${audience} care se regăsesc în situația "${topSituation}" și se lovesc frecvent de problema "${topProblem}". Direcția pune accent pe un proces clar, realist și ușor de urmat către ${topOutcome.toLowerCase()}.`,
    },
    {
      variant: buildSpecificFallbackNicheTitle(audience, secondSituation, secondProblem, topOutcome, 1),
      description: `Pentru ${audience} care au nevoie de un mesaj clar într-un context de tip "${secondSituation}". Varianta rezolvă mai ales blocajul "${secondProblem}" și promite un drum credibil spre ${topOutcome.toLowerCase()}, fără soluții extreme.`,
    },
    {
      variant: buildSpecificFallbackNicheTitle(audience, topSituation, secondProblem, topOutcome, 2),
      description: `Pentru ${audience} care au nevoie de claritate, structură și pași aplicabili în viața reală. Nișa pleacă de la problema "${secondProblem}" și transformă promisiunea de ${topOutcome.toLowerCase()} într-un mesaj mai specific și mai credibil.`,
    },
  ];
}

function buildMockNicheVariants(input: NicheDiscoverPhaseAInput): NicheVariant[] {
  const audience = buildAudienceLabel(input);
  const situations = input.valueSituations
    .map((value) => translateSituationToNicheLanguage(value))
    .filter(Boolean);
  const problems = input.commonProblems
    .map((value) => translateProblemToNicheLanguage(value))
    .filter(Boolean);
  const avoid = input.avoidContent.map((value) =>
    normalizeNicheFragment(value, '')
  ).filter(Boolean);
  const outcome = translateOutcomeToNicheLanguage(input.primaryOutcome);

  const [situationOne, situationTwo, situationThree] = [
    situations[0] || 'au un program aglomerat și puțin timp pentru ele',
    situations[1] || situations[0] || 'se pierd între prea multe informații și soluții contradictorii',
    situations[2] || situations[0] || 'au nevoie de structură clară și pași simpli',
  ];

  const [problemOne, problemTwo, problemThree] = [
    problems[0] || 'abandonează repede pentru că planul pare prea greu',
    problems[1] || problems[0] || 'nu reușesc să fie constante mai mult de câteva săptămâni',
    problems[2] || problems[0] || 'au impresia că fac bine, dar rezultatele întârzie',
  ];

  const avoidClause = avoid.length
    ? ` Comunicarea evită explicit: ${avoid.join(', ')}.`
    : '';

  return [
    {
      variant: `${buildOutcomeNicheLabel(outcome)} pentru ${buildAudienceNicheLabel(audience)} care ${buildProblemNicheLabel(problemOne).toLowerCase()}`,
      description: `Această variantă este condusă de problema dominantă. Se adresează persoanelor din segmentul ${audience} care, deși vor rezultate, ${problemOne.toLowerCase()}. Mesajul central promite ${outcome.toLowerCase()} printr-un proces clar și aplicabil, nu prin motivație de moment.${avoidClause}`,
    },
    {
      variant: `${buildOutcomeNicheLabel(outcome)} pentru ${buildAudienceNicheLabel(audience)} ${buildContextNicheLabel(situationTwo)}`,
      description: `Această variantă este condusă de contextul de viață. Se adresează persoanelor din segmentul ${audience} care trăiesc realitatea de tip "${situationTwo}" și au nevoie de o soluție care să încapă în viața lor, nu de un plan idealizat. Promisiunea este ${outcome.toLowerCase()} într-un mod realist și sustenabil.${avoidClause}`,
    },
    {
      variant: `${buildOutcomeNicheLabel(outcome)} pentru ${buildAudienceNicheLabel(audience)} care vor o schimbare vizibilă fără extreme`,
      description: `Această variantă este condusă de rezultatul dorit. Este pentru persoane din segmentul ${audience} care vor ${outcome.toLowerCase()}, dar într-o formă credibilă, fără promisiuni rapide și fără soluții extreme. Antrenorul este poziționat ca ghid pentru progres constant, chiar dacă există blocaje precum "${problemThree}".${avoidClause}`,
    },
  ];
}

type NicheVariantAngle = 'problem' | 'context' | 'outcome';

function buildNicheVariantPrompt(input: NicheDiscoverPhaseAInput, angle: NicheVariantAngle): string {
  const angleInstruction =
    angle === 'problem'
      ? 'Varianta trebuie să fie condusă în primul rând de problema dominantă a clientului și de blocajul recurent.'
      : angle === 'context'
        ? 'Varianta trebuie să fie condusă în primul rând de contextul de viață al clientului și de situația în care antrenorul aduce cea mai mare valoare.'
        : 'Varianta trebuie să fie condusă în primul rând de rezultatul dorit în următoarele 2-3 luni și de transformarea clară promisă.';

  const forbiddenKeywords =
    angle === 'problem'
      ? 'Nu centra titlul pe program, ghidaj, pași sau contextul de viață. Centrează-l pe problema clară pe care clientul vrea să o depășească.'
      : angle === 'context'
        ? 'Nu centra titlul doar pe problemă sau pe rezultat. Centrează-l pe stilul de viață, contextul și realitatea zilnică a clientului.'
        : 'Nu centra titlul doar pe blocaj sau pe context. Centrează-l pe rezultatul dorit și pe promisiunea clară a nișei.';

  return `Tu ești un expert în marketing fitness. Primești răspunsurile unui antrenor și trebuie să propui EXACT 1 variantă de nișă.
...
UNGHI OBLIGATORIU PENTRU ACEASTĂ VARIANTĂ:
${angleInstruction}

REGULI CRITICE:
- titlul trebuie să sune ca o nișă reală de business
- titlul trebuie să fie clar, natural și suficient de specific
- titlul trebuie să includă explicit:
  1. cine este clientul
  2. ce context sau blocaj are
  3. ce rezultat urmărește
- nu livra 3 variante, ci EXACT una
- nu livra formule vagi sau abstracte
- evită complet formulări de tip:
  - "ghidaj pas cu pas pentru tineri"
  - "program clar pentru femei"
  - "transformare pentru oameni ocupați"
  - "fitness simplificat"
  - "ajutor pentru consecvență"
- transformă răspunsurile brute din quiz în limbaj comercial clar și natural
- dacă răspunsul este abstract, rescrie-l ca nișă concretă
- ${forbiddenKeywords}

FORMAT:
{
  "variant": "titlul nișei",
  "description": "2-3 propoziții clare despre client, problemă și ce rezolvă antrenorul"
}

Răspunde DOAR JSON valid, fără markdown.`;
}

async function generateSingleNicheVariant(
  input: NicheDiscoverPhaseAInput,
  angle: NicheVariantAngle,
  index: number
): Promise<NicheVariant | null> {
  const prompt = buildNicheVariantPrompt(input, angle);
  const content = await generateAnthropicJson(prompt, 0.7, 450);
  const parsed = await parseModelJson<any>(content);

  return normalizeNicheVariantEntry(parsed, index);
}

export async function generateNicheVariants(input: NicheDiscoverPhaseAInput): Promise<{ variants: NicheVariant[] }> {
  if (MOCK_AI_RESPONSES) {
    return { variants: buildMockNicheVariants(input) };
  }

  const generatedResults = await Promise.allSettled([
    generateSingleNicheVariant(input, 'problem', 0),
    generateSingleNicheVariant(input, 'context', 1),
    generateSingleNicheVariant(input, 'outcome', 2),
  ]);

  const variants: NicheVariant[] = [];
  ...
  generatedResults.forEach((result, index) => {
    ...
    const titleKey = variant.variant.toLowerCase();
    if (looksTooGenericNicheVariantTitle(variant.variant) || usedTitles.has(titleKey)) {
      return;
    }
  ...
  for (const fallback of buildFallbackNicheVariants(input)) {
    ...
    const fallbackTitle = fallback.variant.toLowerCase();
    if (usedTitles.has(fallbackTitle) || looksTooGenericNicheVariantTitle(fallback.variant)) {
      continue;
    }
