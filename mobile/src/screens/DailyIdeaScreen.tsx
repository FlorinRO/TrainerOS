...
function getDailyIdeaErrorMessage(error: any): { title: string; body: string } {
  ...
  if (status === 429 || status === 403) {
    return {
      title: 'Limită atinsă',
      body: rawMessage || 'Ai atins limita curentă pentru Daily Idea.',
    };
  }
  ...
  if (status === 400 && error?.response?.data?.nicheRequired) {
    return {
      ...
      body: rawMessage || 'Setează mai întâi nișa în Niche Finder.',
    };
  }
  ...
  if (status === 401) {
    return {
      ...
      body: 'Te rugăm să te autentifici din nou.',
    };
  }
  ...
  if (message.includes('timed out') || message.includes('timeout') || message.includes('abort')) {
    return {
      ...
      body: 'Serviciul AI a răspuns prea lent. Încearcă din nou.',
    };
  }
  ...
  return {
    title: 'Eroare',
    body: rawMessage || 'Nu am putut genera ideea.',
  };
}
...
export default function DailyIdeaScreen() {
  ...
  const generateMutation = useMutation({
    ...
    onError: async (error: any) => {
      ...
      if (status === 429 || status === 403) {
        const errorCopy = getDailyIdeaErrorMessage(error);
